...
All generated artwork is a first production pass, pending review. Only the wizard's directional sprites, walk and cast frames (via `../tools/copy_character_frames.py`), a breathing idle built from the stills (`../tools/make_breathing_idle.py`; the generated idle clips were too exaggerated), with walk frames pinned to the ground line by `../tools/ground_lock_frames.py`, and three enemy sprites are currently copied into `../game`; generating another asset does not add it to the game.
